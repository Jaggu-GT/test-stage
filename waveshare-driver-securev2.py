#!/usr/bin/env python3
"""
waveshare-driver-securev2.py — from-scratch driver + long-running display daemon for the
...
class EPD2in7V2:
    ...
    def _reset(self) -> None:
        self._rst.on();  time.sleep(0.020)
        self._rst.off(); time.sleep(0.002)
        self._rst.on();  time.sleep(0.020)

    def _power_on_init(self) -> None:
        # Runs under the bus lock; uses the SHORT busy timeout so a dead panel
        # can hold the bus for at most a few seconds, not 30.
        self._reset()
        self._wait_busy(_INIT_BUSY_TIMEOUT_S)
        self._command(0x12)                        # SWRESET
        self._wait_busy(_INIT_BUSY_TIMEOUT_S)
        self._command(0x74); self._data(0x54)      # analog block control
        self._command(0x7E); self._data(0x3B)      # digital block control
        self._command(0x01)                        # driver output: (264-1)=0x0107
        self._data(0x07, 0x01, 0x00)
        self._command(0x11); self._data(0x03)      # data entry: X+ Y+
        self._command(0x44); self._data(0x00, 0x15)            # RAM-X 0..21
        self._command(0x45); self._data(0x00, 0x00, 0x07, 0x01)  # RAM-Y 0..263
        self._command(0x3C); self._data(0x03)      # border waveform
        self._command(0x18); self._data(0x80)      # built-in temp sensor
        self._command(0x21); self._data(0x00, 0x80)  # display update control
        self._command(0x4E); self._data(0x00)      # RAM-X counter -> 0
        self._command(0x4F); self._data(0x00, 0x00)  # RAM-Y counter -> 0
        self._wait_busy(_INIT_BUSY_TIMEOUT_S)
    ...
    def refresh(self, buf: bytes, *, force: bool = False) -> bool:
        ...
        # Phase 1: clock the image + kick the waveform, under the lock.
        with Spi0BusLock(self._lock_policy, self._lock_timeout):
            ...
            self._send_update()
        try:
            # Phase 2: bus FREE — panel updates autonomously (~6s).
            self._wait_busy(_BUSY_TIMEOUT_S)
        except EpdError as e:
            wait_error = e
        finally:
            # Phase 3: deep-sleep, best-effort, patient (never leave panel powered).
            try:
                with Spi0BusLock("wait", _SLEEP_LOCK_WAIT_S):
                    self._sleep_panel()
            except Spi0Busy:
                # Bus stayed busy unusually long; panel may remain powered.
                # Surface a warning, but preserve the original hardware fault below.
    ...
    def getbuffer(image) -> bytes:
        """Convert a PIL 1-bit image (WIDTHxHEIGHT or rotated) to a panel buffer."""
        img = image.convert("1")
        w, h = img.size
        if (w, h) == (WIDTH, HEIGHT):
            framed = img
        elif (w, h) == (HEIGHT, WIDTH):
            framed = img.rotate(90, expand=True)
        else:
            raise ValueError(f"image must be {WIDTH}x{HEIGHT} or {HEIGHT}x{WIDTH}")
        return bytes(framed.tobytes())
