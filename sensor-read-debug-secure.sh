...
#   - user has access to /dev/i2c-1 and /dev/spidev0.0
#     Prefer a dedicated board-access group assigned by udev over broad
#     bus-wide i2c/spi group membership; see 99-coral-env-debug.rules.example.
#
# Usage:
...
#   - validates raw I2C bytes before arithmetic conversion
...
validate_i2c_hex_bytes() {
    local label="$1"
    shift
    local byte
    for byte in "$@"; do
        if [[ ! $byte =~ ^0x[0-9a-fA-F]{1,2}$ ]]; then
            warn "  malformed I2C output from $label"
            return 1
        fi
    done
}
...
preflight() {
    ...
    if [[ ! -r /dev/i2c-$BUS  || ! -w /dev/i2c-$BUS  ]] && [[ $EUID -ne 0 ]]; then
        fail "/dev/i2c-$BUS not user-accessible — install 99-coral-env-debug.rules.example and add user to coral-env-debug"
    fi
    if [[ ! -r /dev/spidev0.0 || ! -w /dev/spidev0.0 ]] && [[ $EUID -ne 0 ]]; then
        fail "/dev/spidev0.0 not user-accessible — install 99-coral-env-debug.rules.example and add user to coral-env-debug"
    fi
...
read_hdc2010() {
    ...
    validate_i2c_hex_bytes "HDC2010" "$t_lsb" "$t_msb" "$rh_lsb" "$rh_msb" \
        || { echo "-- --"; return 1; }
    local tl=$((16#${t_lsb#0x})) tm=$((16#${t_msb#0x}))
    local rl=$((16#${rh_lsb#0x})) rm=$((16#${rh_msb#0x}))
    awk -v tl="$tl" -v tm="$tm" -v rl="$rl" -v rm="$rm" '
    BEGIN {
...
read_opt3002() {
    ...
    local msb lsb
    read -r msb lsb <<< "$raw"
    validate_i2c_hex_bytes "OPT3002" "$msb" "$lsb" \
        || { echo "--"; return 1; }
    local m=$((16#${msb#0x})) l=$((16#${lsb#0x}))
    awk -v m="$m" -v l="$l" '
    BEGIN {
        w = m*256 + l
