...
for arg in "$@"; do
    case "$arg" in
        --loop)          LOOP_MODE="forever" ;;
        --cycle)         shift; CYCLE_SECONDS="$1" ;;
        --cycle=*)       CYCLE_SECONDS="${arg#*=}" ;;
        --test-pattern)  TEST_PATTERN_ONLY=1 ;;
        --help|-h)
...
preflight() {
    ...
    for cmd in i2cdetect i2ctransfer i2cset awk timeout python3; do
    ...
    python3 -c "import spidev, gpiozero, PIL" 2>/dev/null \
